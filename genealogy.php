...
Plugin URI: http://www.seodenver.com/family-tree/
			...
			<div class="metabox-holder">	
				<div class="meta-box-sortables">
				   ...
				   <?php 
						...
						$checked = (empty($show_credit) || $show_credit == 'yes') ? ' checked="checked"' : '';
						
						$rows[] = array(
								'id' => 'genealogy_show_credit',
								'label' => __('Give credit', 'genealogy'),
								'desc' => 'Checking the box tells the world you use this free plugin by adding a link right under the single-person\'s data display.',
								'content' => "<p><label for='genealogy_show_credit'><input type='hidden' name='genealogy[show_credit]' value='no' /><input type='checkbox' name='genealogy[show_credit]' value='yes' id='genealogy_show_credit' {$checked} /> Give thanks for this free plugin</label></p>"
						);
...
function genealogy_do_settings_filters() {
	...
	if(!empty($settings['show_relationship_thumbnails']) && $settings['show_relationship_thumbnails'] == 'no') {
	...
	if(!empty($settings['show_credit']) && $settings['show_credit'] == 'no') {
		add_filter('genealogy_show_credit', 'genealogy_returnfalse'); 
	}
}
...
function genealogy_init() {
	...
	$args = array(
	  ...
	  'menu_icon' => GENEALOGY_PLUGIN_URL .'images/group.png', // Icon courtesy of famfamfam.com Silk Icon set http://www.famfamfam.com/lab/icons/silk/
...
function genealogy_head() {
	...
	if(is_admin()) {
		...
		if($pagenow == 'edit.php' && isset($_GET['post_type']) && $_GET['post_type'] == 'family') { 
			wp_enqueue_style( 'genealogy-admin', GENEALOGY_PLUGIN_URL.'css/genealogy-admin.css');
		} elseif(is_object($post) && $post->post_type == 'family') { // Single page
			wp_enqueue_script('genealogy-datepicker', GENEALOGY_PLUGIN_URL.'js/jquery-ui-1.8.11.custom.min.js', NULL, array('jquery', 'jqueryui'));
			wp_enqueue_style( 'genealogy-datepicker', GENEALOGY_PLUGIN_URL.'css/smoothness/jquery-ui-1.8.11.custom.css');
			wp_enqueue_script('genealogy-admin', GENEALOGY_PLUGIN_URL.'js/genealogy-admin.js', NULL, array('jquery'));
			wp_enqueue_style( 'genealogy-admin', GENEALOGY_PLUGIN_URL.'css/genealogy-admin.css');
		}
	} else {
		wp_enqueue_style('genealogy', GENEALOGY_PLUGIN_URL.'css/genealogy.css');
	}
}
...
if(is_admin()) { return $content; }
	...
	$output .= $children.get_genealogy_credit();
	return $output;
}

function get_genealogy_credit() { 
	global $post;
	if($post && !is_admin() && apply_filters('genealogy_show_credit', true)) {
		$default = '<p style="text-align:center; margin:1em 0; color:#999;"><small style="font-size:11px;">Created by <a style="color:#777;" href="http://www.wordpress.org/extend/plugins/genealogy/" rel="nofollow">Genealogy</a>, provided by <a href="http://www.melickprogenealogists.com" style="color:#777;">Professional Genealogists</a></small></p>';
		$url = 'http://www.katzwebservices.com/development/attribution.php?site='.esc_html(str_replace('http://', '', str_replace('https://', '', site_url()))).'&from=genealogy&version='.GENEALOGY_PLUGIN_VERSION;
		if(function_exists('fetch_feed')) { // WP Version > 2.8
			@include_once(ABSPATH . WPINC . '/feed.php'); 
			$rss = fetch_feed($url); if (!$rss || is_wp_error($rss) ) { return $default; }
			$rss->strip_attributes(array('bgsound','expr','onclick','onerror','onfinish','onmouseover','onmouseout','onfocus','onblur','lowsrc','dynsrc')); 
			$rss->set_cache_duration(60*60*24*60); // Fetch every 60 days
			$rss_items = $rss->get_items(0, 1);
			return str_replace(array("\n", "\r"), ' ', $rss_items[0]->get_description());
		} else { // WP Version < 2.8
			@include_once(ABSPATH . WPINC . '/rss.php');
			$rss = fetch_rss($url); if (!$rss || is_wp_error($rss) || !is_array($rss->items) || empty($rss->items)) { return $default; }
			$rss->items = array_slice($rss->items, 0, 1);
			return empty($rss->items[0]['description']) ? '' : str_replace(array("\n", "\r"), ' ', (string)$rss->items[0]['description']);
		}
	}
}

function genealogy_credit() { echo get_genealogy_credit(); return; }
...
function genealogy_do_shortcode($atts = array(), $content= null) {
	...
	return $content.get_genealogy_credit();
}
